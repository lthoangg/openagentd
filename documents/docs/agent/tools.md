...
| `generate_video` | `video.py` | Generate a video clip in the session workspace. Four input modes (text-to-video, image-to-video, first+last-frame interpolation, reference images). Returns markdown `![prompt](filename.mp4)`; the frontend's `MarkdownVideo` detects the `.mp4`/`.webm`/`.mov`/`.m4v` extension and renders `<video controls>` via the same `/api/team/{sid}/media/` proxy. |
...
| `images` | `list[str] \| None` | `None` | Workspace-relative path of the **first frame** (exactly 1 entry). Switches to image-to-video mode. |
| `last_frame` | `str \| None` | `None` | Workspace-relative path of the **last frame**. Requires `images` to be set. Together they trigger first→last-frame interpolation. Mutually exclusive with `reference_images`. |
| `reference_images` | `list[str] \| None` | `None` | Workspace-relative paths (up to 3) of subject/style references. Forwarded to Veo as `referenceImages` with `referenceType: "asset"`. Mutually exclusive with `last_frame`. |
| `aspect_ratio` | `Literal["16:9", "9:16"] \| None` | `None` | Per-call override. Falls back to `video.aspect_ratio` in `multimodal.yaml`. |
| `resolution` | `Literal["720p", "1080p", "4k"] \| None` | `None` | Per-call override. `1080p` / `4k` only support 8s duration. Falls back to `video.resolution`. |
| `duration_seconds` | `Literal["4", "6", "8"] \| None` | `None` | Per-call override. Must be `"8"` for `1080p`/`4k`/reference images. Falls back to `video.duration_seconds`. |

Mode is implicit: no inputs → text-to-video; `images=[x]` → image-to-video; `images=[x]` + `last_frame=y` → interpolation; `reference_images=[…]` → subject/style references. Same `video.model` drives all modes. Invalid enum values, mixed-mode inputs (e.g. `reference_images` + `last_frame`), and sandbox path errors (missing, not-a-file, symlink, >5 MB) all return a framed `Error: ...` string **before any HTTP call**.
...
- **`googlegenai`** (video, `backends/googlegenai_video.py`) — shares the `GOOGLE_API_KEY` lookup with the image backend but targets the Veo `predictLongRunning` endpoint. Single entry point `generate(cfg, prompt, *, image, last_frame, reference_images, overrides)` — all inputs are `(filename, raw_bytes)` tuples; the dispatcher owns sandbox resolution. Understands `aspect_ratio` / `resolution` / `duration_seconds` / `person_generation` / `negative_prompt` / `seed` (forwarded as camelCase `parameters.{...}`); silently ignores image-shaped keys. Flow: POST `predictLongRunning` → poll `GET operations/{name}` every 10s until `done=true` → extract `response.generateVideoResponse.generatedSamples[0].video.uri` → download mp4 with `x-goog-api-key` header (follows redirects). Hard wall-clock cap is 10 min.
